  ...
  return (
    <div className="px-4 py-5 max-w-lg mx-auto pb-24">
      <header className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-forest-900 mb-1">How do you eat?</h1>
        <p className="text-sage-600 text-sm">Food systems produce about a third of global emissions. Your choices matter.</p>
      </header>

      {/* Diet type */}
      <fieldset className="mb-6">
        <legend className="block text-sm font-semibold text-forest-800 mb-3">Best describes your diet</legend>
        <div className="grid grid-cols-2 gap-2">
          {DIET_TYPES.map((d) => (
            ...
            >
              ...
              <div>
                <div className={`text-sm font-semibold ${data.dietType === d.value ? 'text-forest-700' : 'text-forest-900'}`}>{d.label}</div>
                <div className="text-xs text-sage-500">{d.desc}</div>
              </div>
            </button>
          ))}
        </div>
      ...
      {/* Local food */}
      <div className="mb-6">
        <label htmlFor="local-food" className="block text-sm font-semibold text-forest-800 mb-1">
        ...
        <p className="text-xs text-sage-500 mb-3" id="local-food-hint">Local food typically has lower transport emissions</p>
        <input
          id="local-food"
          type="range"
          min={0}
      ...
      <fieldset className="mb-8">
        <legend className="block text-sm font-semibold text-forest-800 mb-3">How much food do you waste?</legend>
          ...
          {WASTE_LEVELS.map((w) => (
            ...
            >
              ...
              <div className="text-center">
                <div className={`text-xs font-semibold ${data.foodWasteLevel === w.value ? 'text-forest-700' : 'text-forest-900'}`}>{w.label}</div>
                <div className="text-xs text-sage-400 mt-0.5">{w.desc}</div>
              </div>
            </button>
          ))}
        </div>
